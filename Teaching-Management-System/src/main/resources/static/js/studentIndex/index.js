...
(function () {
  ...
  request.get('/manangmentSystem/user/getUser?number=001').then((res) => {
    ...
    let info = {
      ...
      sid: res.data.data.number,
      class: res.data.data.grade + res.data.data.clazz,
      dept: res.data.data.major,
      phone: res.data.data.phoneNumber,
    };
    // console.log()
    let html = template('stuInfo', {
      obj: info,
    });
  ...
  updateImg.addEventListener('click', () => {
    if (updateFlag) {
      ...
      if (flag) {
        ...
        formData.append('fileName', fileBtn.files[0]);
        formData.append('uid', '3225');
        ...
        /*程世玉修改 2022.02.19*/
        axios.post('/manangmentSystem/user/updateHeadImage',formData,{
          headers:{
  ...
  let uid= '';
  let uids = '';
  let guideStuSearch = document.querySelector('.stuSearch');
  let constu =  document.querySelector('.constu');
  let stuInfo = document.querySelector('.stuInfo');
  let  sname = document.querySelector('#contest-name');//指导老师名称
  guideStuSearch.addEventListener('click',function(ev){
      var oEvent = ev || window.event;
      // 查询获奖人姓名
      // let snames = sname.value.split(',');
    /*程世玉修改 2022.02.19*/
    let snames = sname.value;
    request
        .request({
          url: '/manangmentSystem/prize/getPrizeWinner',
          // params: { username: snames},
          params: { usernameList: snames},
        ...
        stuInfo.innerHTML = guideTeaInfo;

        constu.style.display='block';
        oEvent.cancelBubble = true;

        var teaLis = stuInfo.querySelectorAll('li');
        ...
        for (let i = 0; i < teaLis.length; i++) {
          teaLis[i].addEventListener('click', function() {
            ...
            let content = sname.value;
            let reg = eval(`/${ss}/ig`);
            if(!reg.test(content)) {
              if(flag) {
                sname.value += ss;
                uid += this.children[2].innerHTML ;
                console.log('进去了');
              ...
              for(let i=0; i<newNames.length; i++) {
               if(this.children[0].innerHTML == newNames[i].innerHTML) {
                  uids = uid.replaceAll(`${newNames[i].parentNode.children[2].innerHTML}`,this.children[2].innerHTML);
                  console.log(uids);
               }
              }
            }
      ...
      // 对ul取消冒泡事件
      stuInfo.addEventListener('click', function(ev){
        var oEvent = ev || window.event;
        oEvent.cancelBubble = true;
  ...
  let  tname = document.querySelector('#adsive-tea');//指导老师名称
  guideTeaSearch.addEventListener('click',function(ev){
    var oEvent = ev || window.event;
    // 查询获奖人姓名
    //   let names = tname.value.split(',');
    /*程世玉修改 2022.02.19*/
    let names = tname.value;

    request
        .request({
          url: '/manangmentSystem/prize/getPrizeWinner',
          params: { usernameList: names},
          method: 'get',
        })
        .then((res) => {
          console.log('查询获奖人名字', res);
...
// 自定义弹出信息框 
  // function alertDiv(div,msg) {
  //   let alertDivHeader = document.createElement('p');
  //   div.appendChild(alertDivHeader);
  //   alertDivHeader.innerHTML = '提示';
  ...
  uploadBtn.addEventListener('click', function () {
    ...
    if (flag) {
      ...
      let tname = document.querySelector('#adsive-tea').value; //指导老师名称
      ...
      //   });
        ...
        /*程世玉修改 2022.02.20*/
        axios.post('/manangmentSystem/fileOperate/uploadPrize',form,{
            headers:{
                'content-type':'multipart/form-data'
            }
        }).then((res) => {
